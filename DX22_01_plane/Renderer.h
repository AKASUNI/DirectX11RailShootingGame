...
struct LIGHT {
	BOOL Enable;							//有効状態
	BOOL Dummy[3];							
	DirectX::SimpleMath::Vector4 Direction; //向き
	DirectX::SimpleMath::Color   Diffuse;	//強さ、色
	DirectX::SimpleMath::Color   Ambient;	//環境光の強さ、色
};
...
private:
	...
	static ID3D11Device*           m_pDevice;				//GPUに送る用のデータを作成する
	static ID3D11DeviceContext*    m_pDeviceContext;		//送られたデータを元にGPUに描画命令
	...
	static ID3D11Buffer*			m_pTextureBuffer;		//uv指定
